...
C..........INPUT RUN SPECIFICATIONS.
 ...
 2314         FORMAT (' ','Run from ',1pe12.5,' to ',1pe12.5,
     |                    ' in increments of ',1pe12.5)
              WRITE (iw,2316)
 2316         FORMAT (' ','Confirm these values (1 or 0): ',$)
              READ (ir,2301) lchose                         !Get confirmation.
 2301         FORMAT (a1)
              IF ((lchose.ne.0)) GO TO 231
...
C..........DISPLAY SCREEN OUTPUT SELECTIONS.
 ...
 3000   FORMAT (8(/),
     ...
     |          ' ',26x,'------ ------ ---------',//,
     |          ' ',25x,' 1. DISPLAY D,T,HE3,HE4,LI7,HeH',/,
...
C..........PRINT CAPTION.
 ...
 3102     FORMAT (' ','Model parameters:',/,
     ...
     |            ' ','   xi-e = ',e10.3,'/ xi-m = ',e10.3,
     |                '/ xi-t = ',e10.3,/)
C..........PRINT HEADINGS, ABUNDANCES FOR D,T,HE3,HE4,LI7,HeH
          WRITE (iw,3104)
 3104     FORMAT (4x,'Temp',8x,'D/H',9x,'T/H',8x,'He3/H',8x,
     |            'He4',8x,'Li7/H',8x,'HeH/H',/,' ',80('-'))
          DO j = 1,it
            WRITE (iw,3106) t9out(j),(xout(j,i),i=3,6),xout(j,8),
     |                      xout(j,27)
 3106       FORMAT (1pe9.3,1p5e11.3)
          END DO
          WRITE (iw,2014)
          WRITE (iw,3108)
